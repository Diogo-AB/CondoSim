# CondoSim


## VISÃO DO PRODUTO

*Para os condomínios que não têm síndico*  
*Cujo problema é a gestão de recursos financeiros e manutenção*  
*O CondSin*  
É um aplicativo móvel  
Que facilita o gerenciamento do condomínio  
Diferentemente de um síndico  
Nosso produto simplifica a gestão do condomínio, sempre mantendo os habitantes informados

---

## PRODUTO É / NÃO É / FAZ / NÃO FAZ

*O produto é* 
Aplicativo mobile
Multiplataforma
Gratuito
Facilitador da gestão do condomínio

*O produto não é*
Síndico
Um grupo de moradores
Planilha

*O produto faz*
Organiza os recursos financeiros, horários e manutenção do  condomínio
Notifica moradores

*O produto não faz*
Contratação de prestadores de serviços
Pagamento de contas

---

## PERSONA

### MÁRCIA – MÃE TRABALHADORA

*Perfil:*  
- 38 anos  
- Divorciada, com filho  
- Analista de sistemas  
- Formada  

*Comportamento:*  
- Proativa  
- Organizada  
- Comunicativa  
- Tecnóloga  
- Responsável  

*Necessidades:*  
- Organizar as contas a pagar do condomínio  
- Se organizar com base na agenda do condomínio  

---

## BRAINSTORM DE FUNCIONALIDADES

- Agenda de serviços gerais  
- Mural virtual  
- Gera e envia relatório de gastos do condomínio para moradores  
- Tela principal com avisos e gastos  
- Área de pagamentos  
- Mapa de moradores  
- Informações gerais do prédio  
- Calendário do Condomínio  
- Painel Financeiro  

---

## REVISÃO TÉCNICA, DE NEGÓCIOS E DE UX

| Funcionalidade           | Confiança | Esforço | UX             | Valor de Negócio | Observação |
|--------------------------|-----------|---------|----------------|------------------|------------|
| Agenda de Serviços Gerais| Amarelo   | EE      | 🖤🖤🖤        | $$               |            |
| Mural Virtual            | Verde     | E       | 🖤🖤🖤        | $$               |            |
| Relatório                | Vermelho  | EEE     | 🖤🖤          | $$$              |            |
| Agenda do Prédio         | Amarelo   | EE      | 🖤🖤🖤        | $$               |            |
| Tela Principal           | Verde     | E       | 🖤🖤🖤        | $$$              |            |
| Área de Pagamentos       | Amarelo   | EE      | 🖤🖤          | $$               |            |
| Mapa de Moradores        | Verde     | E       | 🖤🖤🖤        | $$$              |            |
| Informações Gerais       | Verde     | E       | 🖤             | $                |            |
| Painel Financeiro        | Amarelo   | EE      | 🖤🖤🖤        | $$               |            |

---

## JORNADA DO USUÁRIO

*Márcia – Mãe Trabalhadora*

- *06:00*  
  Márcia acorda e, tomando café, abre a *Tela Principal* do CondSin. Lá confere rapidamente os avisos recentes e o resumo dos gastos do mês.

- *06:30*  
  Acessa a *Agenda de Serviços Gerais* e verifica se haverá alguma manutenção no prédio durante o fim de semana.

- *07:00*  
  Enquanto leva o filho para a escola, recebe uma notificação sobre um aviso de barulho ocorrido no andar de cima fora do horário permitido.

- *12:30*  
  Durante o treino na academia, é avisada por uma notificação automática de que a tarifa de água aumentou no mês e o condomínio recomenda economia.

- *17:00*  
  Esperando no carro após buscar o filho, Márcia abre o *Calendário do Condomínio* para checar se há alguma reunião agendada e aproveita para acessar o *Mural Virtual*, onde lê a sugestão de outro morador sobre melhorias na garagem.

- *17:10*  
  Recebe uma notificação confirmando a leitura da sugestão feita no mural por outros moradores e vê que sua participação é registrada.

- *20:00*  
  Já em casa, acessa a *Área de Pagamentos* e confirma que o boleto do mês já está quitado. Logo em seguida, recebe por e-mail o *Relatório de Gastos do Condomínio* e visualiza no app os detalhes no *Painel Financeiro*.

---

## FUNCIONALIDADES NA JORNADA

| Passo                                          | Funcionalidade                        |
|------------------------------------------------|---------------------------------------|
| Acorda, toma café e acessa o sistema           | Tela Principal                        |
| Verifica manutenção                            | Agenda de Serviços Gerais             |
| Recebe notificação de barulho                  | Notificações                          |
| Notificação sobre economia                     | Notificações                          |
| Verifica calendário e mural                    | Calendário, Mural Virtual             |
| Confirma leitura no mural                      | Notificações                          |
| Paga boleto e acessa relatório                 | Área de Pagamentos, Painel Financeiro |

---

## SEQUENCIADOR

| Funcionalidade        | Onda | MVP |
|-----------------------|------|-----|
| Agenda de Locais      | 1    | 1   |
| Mural Virtual         | 1    | 1   |
| Relatório             | 1    | 1   |
| Tela Principal        | 1    | 1   |
| Agenda do Prédio      | 2    | 1   |
| Área de Pagamentos    | 2    | 0   |
| Mapa de Moradores     | 3    | 0   |
| Informações Gerais    | 3    | 0   |
| Painel financeiro     | 3    | 0   |

---

## CANVAS MVP

### PERSONAS SEGMENTADAS:

- *Márcia – Mãe Trabalhadora*  
  Usa agenda e notificações no dia a dia.

- *João – Administrador*  
  Gerencia serviços gerais, relatórios e advertências.

- *Carlos – Morador Sênior*  
  Consulta boletos e informações gerais.

- *Luiza – Jovem Profissional*  
  Reserva espaços e acompanha gastos via app.

### PROPOSTA DO MVP:

- Reserva de áreas comuns (agenda).
- Envio de notificações a moradores.
- Geração e envio de relatórios de gastos.
- Consulta de boletos na área de pagamentos.
- Dashboard inicial com avisos e resumo financeiro.

### RESULTADO ESPERADO:

- *Engajamento:* +70% dos moradores ativos mensalmente.
- *Redução de tarefas sindicais:* 50% (informações automatizadas).
- *Transparência financeira:* feedback positivo em pesquisa de satisfação ≥ 4/5.

### JORNADAS:

- *Márcia – Mãe Trabalhadora*  
  (ver tabela anterior)

- *João – Morador Recente*  
  Acessa dashboard → agenda serviços gerais → envia advertência → gera relatório → envia e-mail.

- *Carlos – Morador Sênior*  
  Recebe e-mail de gastos → acessa área de pagamentos → quita boleto → confirma pagamento.

### FUNCIONALIDADES:

- *Tela Principal:* avisos, gastos, notificações.
- *Agenda de Locais:* reserva de salão, piscina etc.
- *Notificações/Advertências:* push e e-mail para moradores.
- *Área de Pagamentos:* emissão e consulta de boletos.
- *Relatórios de Gastos:* geração e envio automático.
- *Mapa de Moradores:* visualização interativa por andar.
- *Informações Gerais:* regras, contatos, horários.

### MÉTRICAS PARA VALIDAR O NEGÓCIO:

- *Taxa de adoção:* % de moradores cadastrados vs. Total.
- *Taxa de uso:* número médio de reservas/morador/mês.
- *Tempo de resposta:* latência de notificação < 1 min.
- *Satisfação:* NPS e CSAT após 3 meses.

---

## PRODUCT BACKLOG BUILDING - PBB CANVAS  
**Nome do Produto:** CondoSim

### Problemas
- Falta de organização nas tarefas do condomínio
- Informações descentralizadas (avisos, reservas, gastos)
- Dificuldade em acessar relatórios financeiros
- Falta de comunicação entre moradores
- Moradores não sabem sobre reuniões ou manutenções
- Gestão de pagamentos é confusa
- Sem histórico acessível de eventos e gastos
- Nenhuma padronização na comunicação

### Expectativas
- Centralizar informações do condomínio em um só lugar
- Aumentar a transparência financeira
- Permitir que moradores acompanhem gastos mensais
- Notificar sobre reuniões, manutenções e eventos
- Criar um painel intuitivo com informações úteis

### Personas
- **Márcia – Mãe Trabalhadora:** Organizada, quer praticidade no acesso a avisos e pagamentos
- **João – Administrador:** Responsável por relatórios e avisos, deseja simplificar a comunicação
- **Carlos – Morador Sênior:** Precisa de uma forma simples de ver boletos e avisos
- **Luiza – Jovem Empreendedora:** Gosta de acompanhar as finanças de forma digital

### Funcionalidades (1ª Onda)
- Tela Principal com avisos e resumo financeiro
- Agenda de eventos(reuniões, assembleias e etc)
- Mural Virtual com avisos e sugestões
- Relatório de Gastos mensal acessível
- Notificações automáticas e interativas

### PBIs (Product Backlog Items)
- Criar Tela Principal com lista de avisos
- Implementar Agenda de reuniões e assembleias 
- Criar Mural Virtual com suporte a comentários
- Gerar Relatório de Gastos mensal para download e visualização
- Desenvolver sistema de Notificações push e por e-mail

## Protótipo de Baixa Qualidade

![Protótipo de Baixa Qualidade](https://github.com/Diogo-AB/CondoSim/blob/main/prot%C3%B3tipo%20de%20baixa%20qualidade/prot%C3%B3tipo%20de%20baixa%20qualidade.jpg)

## Protótipo de Alta Qualidade

![Protótipo de Alta Qualidade](https://github.com/Diogo-AB/CondoSim/blob/main/prot%C3%B3tipo%20de%20alta%20qualidade/prot%C3%B3tipo%20de%20alta%20qualidade%20.jpg)

## Diagrama de Classes de Domínio

![Diagrama de Classes de Domínio](https://github.com/Diogo-AB/CondoSim/blob/main/diagrama%20de%20classes%20de%20dom%C3%ADnio/diagrama%20de%20classes%20de%20dom%C3%ADnio.jpg)
